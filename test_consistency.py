...
from classix_m import CLASSIX_M      # Original pure Manhattan version
from classix_t import CLASSIX_T      # Original pure Tanimoto version
...
X_blob, _ = make_blobs(n_samples=800, centers=5, cluster_std=1.2, random_state=42)
...
run_test(X_blob, metric='manhattan', radius=0.6, minPts=4, mergeScale=1.5)

print("\nTest 2: Moons data (Manhattan)")
run_test(X_moon, metric='manhattan', radius=0.18, minPts=5, mergeScale=1.3)

print("\nTest 3: Circles + noise (Manhattan)")
run_test(X_circle, metric='manhattan', radius=0.22, minPts=6, mergeScale=1.4)
